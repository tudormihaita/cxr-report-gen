...
from constants import BLIP_BERT_TYPE, BLIP_SPECIALIZED_BERT_TYPE
...
class BLIPRetrieval(nn.Module):
    ...
    def forward(self, image, caption, idx, alpha):
        with torch.no_grad():
            self.temp.clamp_(0.001, 0.3)

        idx = idx.to(self.idx_queue.device)
        ...
        idx = idx.view(-1, 1)
        idx_all = torch.cat([idx.t(), self.idx_queue.clone()], dim=1)
...
class BLIPDecoder(nn.Module):
    ...
    def forward(self, image, caption):
        ...
        text = self.tokenizer(caption, padding='longest', truncation=True, max_length=self.max_length, return_tensors="pt").to(
            image.device)
    ...
    def generate(self, image, sample=False, num_beams=3, top_p=0.9,
...
def init_tokenizer():
    tokenizer = BertTokenizer.from_pretrained(BLIP_BERT_TYPE)
